...
demonstration_data = []
save_path = "manual_demonstration_clipped.csv"
...
figure_filenames = ["Figure 6.eps", "Figure 7.eps", "Figure 8.eps", "Figure 9.eps", "Figure 10.eps"]
...
for metric, fig_name in zip(metrics_to_plot, figure_filenames):
    ...
    for name, hist in train_histories.items():
        if name in ["MetaRL (LightGAT+BiACT)"]:
            continue  # Skip these models
        if metric in hist:
            style, marker, color = next(style_cycle)
            values = hist[metric]
            plt.plot(values, label=f"{name} (avg={np.mean(values):.4f})", linestyle=style, marker=marker, color=color, alpha=0.9)
    plt.xlabel("Epoch")
    ...
    for line in plt.gca().get_lines():
        line.set_alpha(1.0)
    # Save or show
    # plt.savefig(f"plots/{fig_name}", format='eps', dpi=1000)
    plt.show()
    plt.close()
